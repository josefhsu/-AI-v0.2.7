        ...
        <div className="relative group aspect-square bg-black rounded-lg overflow-hidden">
            ...
            <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center p-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                    ...
                    <button onClick={() => onSendImageToVeo(image, 'start')} title="用於首幀" className="p-2 text-white bg-cyan-800/80 rounded-full hover:bg-cyan-600 transition-colors"> <SendToStartFrameIcon className="w-5 h-5" /> </button>
                    <button onClick={() => onSendImageToVeo(image, 'end')} title="用於尾幀" className="p-2 text-white bg-cyan-800/80 rounded-full hover:bg-cyan-600 transition-colors"> <SendToEndFrameIcon className="w-5 h-5" /> </button>
                </div>
            </div>
    ...
    <main className="flex-1 flex flex-col p-2 md:p-4 bg-black min-w-0 overflow-y-auto">
      {hasContent ? (
        ...
        <div className="flex flex-col items-center justify-center text-center h-full text-slate-400 p-4">
            <img 
// Fix: Replaced corrupted base64 string with a valid 1x1 transparent PNG to prevent broken image icon.
                src="data:image/png;base64,iVBORw0KGgoAAODm/WyLAAAAAElFTkSuQmCC" 
                alt="鳥巢AI包娜娜 Logo" 
